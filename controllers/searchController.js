...
const MachineRentalModel = require('../modeles/machineRentalModel');
...
exports.universalSearch = catchAsync(async (req, res, next) => {
    ...
    const MachineRentalModelResults = await MachineRentalModel.find({
        $or: [
            { title: searchRegex },
    ...
    res.status(200).json({
        status: 'success',
        data: {
            MachineRentalModels: MachineRentalModelResults,
            orders: orderResults,
            users: userResults,
        }
    });
...
// MachineRentalModel Search
exports.MachineRentalModelSearch = catchAsync(async (req, res, next) => {
    ...
    const MachineRentalModels = await MachineRentalModel.find(query).populate('userId', 'username profilePicture').limit(20);

    res.status(200).json({
        status: 'success',
        results: MachineRentalModels.length,
        data: {
            MachineRentalModels,
        },
    });
